...
4. (Optional) Setup libvirt hook to add/remove port forwarding rules.
   ...
   Hint: use firewall-cmd instead of direct use of iptables to modify rules:

   firewall-cmd --zone=public --remove-forward-port=port=\${Host_port[\$i]}:proto=tcp:toport=\${Guest_port[\$i]}:toaddr=\${Guest_ipaddr}
   firewall-cmd --zone=public --add-forward-port=port=\${Host_port[\$i]}:proto=tcp:toport=\${Guest_port[\$i]}:toaddr=\${Guest_ipaddr}
